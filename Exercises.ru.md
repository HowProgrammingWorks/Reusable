...
массив чисел из диапазона start - end (например, 15 - 30) включая крайние числа.
5. Реализуйте функцию `rangeOdd(start: number, end: number): array` которая
отдает массив нечетных чисел из диапазона из диапазона start - end
(например, 15 - 30) включая крайние числа.
